...
class CesarCipher:
    ...
    def letterToNumber(self, word:str):
        word = word.upper()
        word = word.replace(" ", "0 ")
    ...
    def addLists(self, firstList:list, secondList:list):
        result = []
    ...
    def subtractLists(self, firstList:list, secondList:list):
        result = []
        if len(firstList) == len(secondList):
            for i in range(len(firstList)):
    ...
    def numberToLetter(self, numberListInt:list, oldNumberListStr:list):
        numberListStr = copy.deepcopy(oldNumberListStr)
    ...
    def cesarCipher(self, choice:str="E"):
        ...
        primaryWord, secondaryWord = [self.letterToNumber(word) for word in [primaryWord, secondaryWord]]
        ...
        if choice == "E":
            resultListInt = self.addLists(listPrimaryWord, listSecondaryWord)
        elif choice == "D":
            resultListInt = self.subtractLists(listPrimaryWord, listSecondaryWord)
        else:
            print("Invalid input")
            resultListInt = self.addLists(listPrimaryWord, listSecondaryWord)
        resultListStr = f.intListToStrList(resultListInt)
        resultList = self.numberToLetter(resultListInt, resultListStr)
...
if __name__ == "__main__":
    print(CesarCipher("Chii").cesarCipher("E"))